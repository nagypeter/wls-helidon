...
```
You can use `vi` or `gedit` just execute the following command in the terminal:
```bash
...
```
Copy the following into the new class:
```java
...
{
  "id":null,"firstname":"Peter",
  "lastname":"Nagy","dateofbirth":"09/12/1979",
  "ssn":"123-12-3123",
  "score":593
}
```
...
The container image name already conform to the Registry name so you can use the `push` command to upload. But first you need to login to the registry. For the password copy and paste the OCIR_PASSWORD's (see environment variables) value:
```bash
...
During the update process the `kubectl` configuration file was copied to the proper location.
...
```
The response has to print *Server version* which can be retrieved from the cluster. If the output similar to the above then you are good to go to the next step.
...
```
To deploy the application on Kubernetes you need to have the resource definition which contains the _Deployment_ and _Service_ details. For the Creditscore Microservices Application you can find in the `/u01/content/wls-helidon/k8s/app.creditscore.yaml`:
```yaml
apiVersion: v1
kind: Service
metadata:
...
```
To easily test the running application on Kubernetes you should have to assign LoadBalancer/Public IP to access externally. But it is not recommended to expose internally used microservices endpoints. Instead of make it accessible externally simply create `kubectl port-forward` which is similar to `ssh tunnel` in Kubernetes environment.
